...
<body>
    <div class="header">
        ...
        <div class="subtitle">Laporan Hasil Survei - Dicetak pada {{ now()->translatedFormat('d F Y H:i') }}</div>
    </div>
    ...
    <div class="stats-grid">
        ...
        <div class="stat-box">
            ...
            <div class="stat-value" style="font-size: 12px">
                {{ $survey->start_date->translatedFormat('d M Y') }} -
                {{ $survey->end_date->translatedFormat('d M Y') }}
            </div>
        </div>
        <div class="stat-box">
    ...
    @foreach ($results as $result)
        <div class="question-card">
            <div class="question-header">
                ...
                @endphp
                <span class="question-type">{{ $displayType }}</span>
            ...
            @if (in_array($result['type'], ['single_choice', 'multiple_choice', 'rating']))
                @foreach ($result['data'] as $data)
                    <div class="option-row">
                        <div class="option-label">
                            ...
                            <span style="float: right; font-weight: bold">{{ $data['count'] }}
                ...
                @if ($result['type'] === 'rating' && isset($result['average_rating']))
                    <div style="margin-top: 15px; padding-top: 10px; border-top: 1px dashed #eee">
                        ...
                        <span
                            style="font-size: 18px; font-weight: bold; color: #d97706">{{ $result['average_rating'] }}
                            / 5</span>
                    </div>
                @endif
            @else
                <div style="font-size: 10px; text-transform: uppercase; color: #888; margin-bottom: 5px">10 Jawaban
                    Terbaru:</div>
