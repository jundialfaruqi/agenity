<x-layout title="Edit Pakaian">
    <div class="flex flex-col md:flex-row md:items-center justify-between mb-6">
        <div>
            <h1 class="text-xl font-bold">Edit Pakaian</h1>
            <p class="text-sm text-base-content/60 mt-1">Perbarui data pakaian</p>
        </div>
    ...
    <div class="card bg-base-100 shadow-sm">
        <div class="card-body">
            <form method="POST" action="{{ route('pakaian.update', $pakaian->uuid) }}" class="space-y-6">
                @csrf
                @method('PUT')
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div>
                        ...
                        <select name="kategori_pakaian_id" class="select select-bordered w-full">
                            ...
                            @foreach($kategoris as $kategori)
                                <option value="{{ $kategori->id }}" @selected(old('kategori_pakaian_id', $pakaian->kategori_pakaian_id) == $kategori->id)>
                                    {{ $kategori->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <input name="contoh_pakaian" type="text" value="{{ old('contoh_pakaian', $pakaian->contoh_pakaian) }}"
