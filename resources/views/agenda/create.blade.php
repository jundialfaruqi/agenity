        ...
        <div class="card-body">
            <form action="{{ route('agenda.store') }}" method="POST" class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    ...
                    <div>
                        ...
                        <input name="jenis_agenda" type="text" value="{{ old('jenis_agenda') }}"
                            class="input input-bordered w-full" placeholder="Contoh: Rapat Koordinasi, Seminar, dsb"
                            required>
                        @error('jenis_agenda')
                            <div class="mt-1 text-xs text-error">{{ $message }}</div>
                        @enderror
                    ...
                    <div>
                        ...
                        <select name="master_opd_id" class="select select-bordered w-full" required>
                            ...
                            @foreach ($opds as $opd)
                                <option value="{{ $opd->id }}" @selected(old('master_opd_id') == $opd->id)>{{ $opd->singkatan }} -
                                    {{ $opd->name }}</option>
                            @endforeach
                        </select>
                        @error('master_opd_id')
                    ...
                    <div>
                        ...
                        <select name="visibility" class="select select-bordered w-full" required>
                            <option value="public" @selected(old('visibility') === 'public')>Public (Tampil di dashboard public)
                            </option>
                            <option value="private" @selected(old('visibility') === 'private')>Private (Hanya via link)</option>
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <select name="mode" class="select select-bordered w-full" required>
                            <option value="offline" @selected(old('mode') === 'offline')>Offline (Tatap Muka)</option>
                            <option value="online" @selected(old('mode') === 'online')>Online (Daring)</option>
                            <option value="hybrid" @selected(old('mode') === 'hybrid')>Hybrid (Keduanya)</option>
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <select name="status" class="select select-bordered w-full" required>
                            ...
                            <option value="active" @selected(old('status') === 'active')>Published / Active</option>
                ...
                <div class="flex justify-end gap-3 mt-8">
                    ...
                    <button type="submit" class="btn btn-secondary px-8">Simpan Agenda</button>
                </div>
            </form>
        </div>
    </div>
