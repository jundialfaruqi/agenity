<x-layout title="Master Kategori Pakaian Management">
    <!-- Page Title & Breadcrumbs -->
    <div class="flex flex-col md:flex-row md:items-center justify-between mb-6">
        <div>
            <h1 class="text-xl font-bold">Kategori Pakaian</h1>
            <p class="text-sm text-base-content/60 mt-1">Kelola data kategori pakaian</p>
        </div>
        <div class="text-sm breadcrumbs text-base-content/60">
            <ul>
                <li><a href="{{ route('dashboard.index') }}">{{ $appSetting->app_name ?? config('app.name') }}</a></li>
                <li>Master Pakaian</li>
                <li><a href="{{ route('kategori-pakaian.index') }}"><span class="text-base-content">Kategori
                            Pakaian</span></a></li>
            </ul>
        </div>
    </div>
    ...
    <div class="mb-6">
        <div class="card bg-linear-to-r from-secondary to-neutral text-base-100 p-5">
            <div class="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                <div>
                    <div class="text-lg text-white font-bold">Data Kategori Pakaian</div>
                    <div class="text-sm text-white opacity-80">Monitoring data kategori pakaian sistem</div>
                </div>
    ...
    <div class="flex flex-col sm:flex-row justify-between gap-4 mb-6">
        <div class="form-control">
            <div class="flex flex-col sm:flex-row items-center gap-3">
                <form method="GET" action="{{ route('kategori-pakaian.index') }}" class="flex items-center gap-2">
                    <div class="join">
                        <span
                ...
                <div class="relative w-full sm:w-auto">
                    <input id="kategori-search-input" type="text" placeholder="Search Kategori..."
                        value="{{ request('q') }}"
                    ...
                    <button type="button" id="kategori-search-clear"
                    ...
                    <div id="kategori-search-suggestions"
        ...
        <div class="flex gap-2">
            <a href="{{ route('kategori-pakaian.create') }}" class="btn btn-neutral gap-2">
    ...
    <div class="card bg-base-100 shadow-sm">
                    ...
                    <tbody>
                        @forelse($kategoris as $kategori)
                            <tr>
                                ...
                                <td>
                                    <div class="font-bold text-sm">{{ $kategori->name }}</div>
                                </td>
                                ...
                                <td class="text-center">
                                    <div class="dropdown dropdown-left dropdown-end">
                                        ...
                                        <ul tabindex="0"
                                            ...
                                            <li>
                                                <a href="{{ route('kategori-pakaian.edit', $kategori->uuid) }}">
                                                    Edit
                                                </a>
                                            </li>
                                            <li>
                                                <button type="button" class="text-error"
                                                    onclick="delete_modal.showModal(); document.getElementById('delete-item-name').innerText = '{{ $kategori->name }}'; document.getElementById('delete-form').action = '{{ route('kategori-pakaian.destroy', $kategori->uuid) }}';">
                                                    Delete
                                                </button>
                                            </li>
                                        </ul>
        ...
        @if ($kategoris->hasPages())
            <div class="card-actions justify-between items-center p-4 border-t border-base-200">
                <div class="w-full">
                    {!! $kategoris->appends(request()->query())->onEachSide(1)->links() !!}
                </div>
            </div>
        @endif
    </div>
    ...
    <dialog id="delete_modal" class="modal">
        <div class="modal-box">
            ...
            <p class="text-sm text-base-content/70">Apakah Anda yakin ingin menghapus Kategori <span
    ...
    <script>
        (function() {
            ...
            const box = document.getElementById('kategori-search-suggestions');
            const clearBtn = document.getElementById('kategori-search-clear');
            let timer = null;
            ...
            function show(items) {
                let html = '';
                if (!items.length) {
                    html = '<div class="p-3 text-sm text-base-content/50 italic">No kategori found</div>';
                } else {
                    html = '<ul class="menu p-2">';
                    items.forEach(item => {
                        html +=
                            `<li><a href="{{ route('kategori-pakaian.index') }}?q=${encodeURIComponent(item.name)}">${item.name}</a></li>`;
                    });
                    html += '</ul>';
                }
                box.innerHTML = html;
            ...
            input.addEventListener('input', () => {
                ...
                timer = setTimeout(() => {
                    fetch(`{{ route('kategori-pakaian.suggest') }}?q=${encodeURIComponent(q)}`)
                        .then(res => res.json())
