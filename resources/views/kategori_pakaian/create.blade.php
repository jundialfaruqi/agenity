<x-layout title="Create Kategori Pakaian">
    <div class="flex flex-col md:flex-row md:items-center justify-between mb-6">
        <div>
            <h1 class="text-xl font-bold">Create Kategori Pakaian</h1>
            <p class="text-sm text-base-content/60 mt-1">Tambah kategori pakaian baru</p>
        </div>
    ...
    <div class="card bg-base-100 shadow-sm">
        <div class="card-body">
            <form method="POST" action="{{ route('kategori-pakaian.store') }}" class="space-y-6">
                @csrf
                ...
                <div class="grid grid-cols-1 gap-5">
                    <div>
                        ...
                        <textarea name="keterangan" class="textarea textarea-bordered w-full" rows="3"
                            placeholder="Masukkan keterangan kategori pakaian">{{ old('keterangan') }}</textarea>
                        @error('keterangan')
                            <div class="mt-1 text-xs text-error">{{ $message }}</div>
                        @enderror
                    </div>
