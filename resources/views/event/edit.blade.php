<x-layout title="Edit Event">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between mb-6">
        <div>
            ...
            <p class="text-sm text-base-content/60 mt-1">Perbarui data event kegiatan</p>
        </div>
    ...
    <div class="card bg-base-100 shadow-sm border border-base-200">
        <div class="card-body">
            <form action="{{ route('event.update', $event->uuid) }}" method="POST" class="space-y-6">
                @csrf
                @method('PUT')
                    ...
                    <div class="lg:col-span-2 space-y-6">
                        <div>
                            ...
                            <div class="prose max-w-none">
                                <textarea name="content" id="editor" class="hidden">{{ old('content', $event->content) }}</textarea>
                            </div>
                            @error('content')
                                <div class="mt-1 text-xs text-error">{{ $message }}</div>
                            @enderror
                        ...
                        <div>
                            ...
                            <textarea name="catatan" class="textarea textarea-bordered w-full h-32"
                                placeholder="Masukkan informasi tambahan jika ada">{{ old('catatan', $event->catatan) }}</textarea>
                        </div>
                    </div>
                            ...
                            <div class="grid grid-cols-1 gap-5">
                                <div>
                                    <label class="label"><span class="label-text font-bold mb-2">Jenis
                                            Event</span></label>
                                    <input name="jenis_event" type="text"
                                        value="{{ old('jenis_event', $event->jenis_event) }}"
                                ...
                                <div>
                                    ...
                                    <select name="master_opd_id" class="select select-bordered w-full" required>
                                        ...
                                        @foreach ($opds as $opd)
                                            <option value="{{ $opd->id }}" @selected(old('master_opd_id', $event->master_opd_id) == $opd->id)>
                                                {{ $opd->singkatan }} -
                                                {{ $opd->name }}</option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="grid grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <select name="mode" class="select select-bordered w-full" required>
                                            ...
                                            <option value="hybrid" @selected(old('mode', $event->mode) === 'hybrid')>Hybrid</option>
                                        </select>
                                    </div>
                                    <div>
                                        <label class="label"><span
